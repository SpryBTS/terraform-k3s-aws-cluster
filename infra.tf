...
resource "aws_launch_template" "k3s_server" {
  ...
  network_interfaces {
    ...
    security_groups       = concat([aws_security_group.ingress.id, aws_security_group.self.id, aws_security_group.database.id], var.extra_server_security_groups)
  }
...
resource "aws_autoscaling_group" "k3s_server" {
  ...
  target_group_arns = flatten([aws_lb_target_group.server-ext[*].arn,aws_lb_target_group.server-int[*].arn])
...
resource "aws_autoscaling_group" "k3s_agent" {
  ...
  target_group_arns = flatten([aws_lb_target_group.agent[*].arn,aws_lb_target_group.agent-int[*].arn])
